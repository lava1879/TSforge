...
However, thanks to the work of **[@InvoxiPlayGames](https://github.com/InvoxiPlayGames)** on **[vistaspctl](https://github.com/InvoxiPlayGames/vistaspctl)**, and its later integration into TSforge, this limitation is effectively bypassed, revealing that the underlying SPP loader driver `spldr.sys` exposes a set of **[IOCTLs](https://github.com/InvoxiPlayGames/vistaspctl/blob/5b22c34e93827965b8ab39a7f55586b74a587b9b/README.txt#L29)** that slsvc normally uses to load the protection driver, along with another unused IOCTL to unload it: `SPLDR_IOCTL_STOP_DRIVER_VISTA`. In practice, this means the driver can be safely paused long enough to access the physical store and then restarted afterward, something that would otherwise be impossible on Vista.
