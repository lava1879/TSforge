    ...
    public static class ZeroCID
    {
        ...
        public static void Activate(PSVersion version, bool production, Guid actId)
        {
            ...
            using (IPhysicalStore store = SPPUtils.GetStore(version, production))
            {
                ...
                // Build 6469 uses Win7-style key blocks, not Vista's PKeyBasicInfo
                // Detect this by checking if we can get SppPkeyShortAuthenticator
                if (version == PSVersion.Vista)
                {
                    // Try Win7-style first (for build 6469)
                ...
                // Build 6469 uses Win7-style blocks (SppPkeyShortAuthenticator) not Vista's PKeyBasicInfo
                if (version == PSVersion.Vista)
                {
                    // Try Win7 format first (for build 6469)
                    CRCBlock shortAuth = pkb.GetBlock("SppPkeyShortAuthenticator");
                    if (shortAuth != null)
                    {
                        Logger.WriteLine("Detected Win7-style key blocks (build 6469)");
                        pkeyData = shortAuth.Value;
                    }
                    else
                    {
                        // Fallback to actual Vista format
                        Logger.WriteLine("Detected Vista-style key blocks");
                        CRCBlock basicInfo = pkb.GetBlock("PKeyBasicInfo");
                        if (basicInfo == null)
                        {
                            throw new InvalidDataException("Failed to find product key data in variable bag.");
                        }
                        pkeyData = basicInfo.Value;
                ...
                // Build 6469 uses Win7-style naming
                string phoneVersion = version == PSVersion.Vista ? "7.0" : "7.0";
                Guid indexSlid = version == PSVersion.Vista ? pkeyId : pkeyId;
